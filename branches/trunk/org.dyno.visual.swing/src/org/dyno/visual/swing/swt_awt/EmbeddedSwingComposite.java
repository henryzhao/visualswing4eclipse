...
/**
 * A SWT composite widget for embedding Swing components in a SWT composite
 * within an RCP or standalone-SWT application. The Eclipse platform provides
 * limited support for embedding Swing components through
 * {@link org.eclipse.swt.awt.SWT_AWT}. This class extends that support by
 * <ul>
 ...
 * </ul>
 * <P>
 * If, rather than embedding Swing components, you are integrating with Swing by
 * opening Swing dialogs, see the {@link AwtEnvironment} class.
 * <p>
 * This is an abstract that is normally used by extending it and implementing
 * the {@link #createSwingComponent()} method. For example,
 * 
 * <pre>
 ...
 * 
 * <p>
 * The Swing component is created inside a standard Swing containment hierarchy,
 * rooted in a {@link javax.swing.RootPaneContainer}. The root pane container
 * is placed inside an AWT frame, as returned by
 * {@link org.eclipse.swt.awt.SWT_AWT#new_Frame(Composite)}
 * <p>
 * <b>Note:</b> When you mix components from Swing/AWT and SWT toolkits, there
 * will be two UI event threads, one for AWT, one for SWT. Most SWT APIs require
 * that you call them from the SWT thread. Swing has similar restrictions though
 * it does not enforce them as much as SWT.
 * <p>
 * Applications need to be aware of the current thread, and, where necessary,
 * schedule tasks to run on another thread. This has always been required in the
 * pure Swing or SWT environments, but when mixing Swing and SWT, more of this
 ...
 * </ul>
 * 
 * Of course, as in single-toolkit environments, long-running tasks should be
 * offloaded from either UI thread to a background thread. The Eclipse jobs API
...
public abstract class EmbeddedSwingComposite extends Composite {
	 ...
	 * {@link #createSwingComponent()}. The creation is scheduled
	 * asynchronously on the AWT event thread. This method does not wait for
	 * completion of this asynchronous task, so it may return before
	 * createSwingComponent() is complete.
	 * <p>
	 * The Swing component is created inside a standard Swing containment
	 * hierarchy, rooted in a {@link javax.swing.RootPaneContainer}. Clients
	 * can override {@link #addRootPaneContainer(Frame)} to provide their own
	 * root pane container implementation.
	 * <p>
	 * This method can be called multiple times for a single instance. If an
	 * embedded frame exists from a previous call, it is disposed.
	 * 
	 * @exception SWTException
	 *                <ul>
	...
	/**
	 * Returns the embedded AWT frame. The returned frame is the root of the AWT
	 * containment hierarchy for the embedded Swing component. This method can
