...
public abstract class LayoutAdapter implements IPropertySourceProvider {
	...
	private static HashMap<String, LayoutBean> layoutBeans;
	...
	static {
		...
		layoutBeans = new HashMap<String, LayoutBean>();
	...
	public static LayoutBean getDefaultLayoutBean() {
		LayoutBean bean = layoutBeans.get(DEFAULT_LAYOUT);
		if (bean == null) {
			...
			if (config != null) {
				...
				if (strBean != null) {
					try {
						bean = (LayoutBean) config.createExecutableExtension("layoutBean");
	...
	public abstract void cloneLayout(JPanel panel);
